...
import { RagKnowledgeStudio } from './components/RagKnowledgeStudio.tsx';
...
export default function App() {
  ...
  const [currentView, setCurrentView] = useState<'chat' | 'rag'>('chat');
  ...
  return (
    <div className="flex h-screen flex-col bg-[#F8FAFC] text-slate-900 font-sans selection:bg-[#FF4F00] selection:text-white overflow-hidden">
      ...
      {/* Main Workspace Area */}
      {currentView === 'rag' ? (
        <main className="mx-auto flex w-full max-w-7xl flex-1 flex-col overflow-hidden p-3 sm:p-6">
          <div className="flex-1 bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-xs flex flex-col">
            <RagKnowledgeStudio onBackToChat={() => setCurrentView('chat')} />
          </div>
        </main>
      ) : (
