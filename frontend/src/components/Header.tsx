...
interface HeaderProps {
  ...
  currentView?: 'chat' | 'rag';
  onToggleView?: (view: 'chat' | 'rag') => void;
}
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-slate-200 bg-white shadow-xs">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-8">
        ...
        <div className="flex items-center gap-2">
          {/* Main View Toggle: Chat vs RAG Knowledge Studio */}
          {onToggleView && (
            <div className="flex items-center p-0.5 rounded-xl bg-slate-100 border border-slate-200 mr-1">
              <button
                id="btn-view-chat"
                onClick={() => onToggleView('chat')}
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold transition ${
                  currentView === 'chat'
                    ? 'bg-white text-[#FF4F00] shadow-xs'
                    : 'text-slate-600 hover:text-slate-900'
                }`}
                title="Conversatie Lab & AI Dutch Tutor"
              >
                <MessageSquare className="w-3.5 h-3.5" />
                <span className="hidden sm:inline">Conversatie</span>
              </button>
              <button
                id="btn-view-rag"
                onClick={() => onToggleView('rag')}
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold transition ${
                  currentView === 'rag'
                    ? 'bg-white text-[#FF4F00] shadow-xs'
                    : 'text-slate-600 hover:text-slate-900'
                }`}
                title="RAG Kennisbank & Live Gespreksopname"
              >
                <Database className="w-3.5 h-3.5" />
                <span className="hidden sm:inline">RAG Studio</span>
              </button>
            </div>
          )}
